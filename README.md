...
```bash
# 安装Python依赖
pip install flask funasr werkzeug
...
├── src/
│   ├── app.py             # Flask后端主程序
│   ├── funasr.ino         # ESP32设备固件代码
│   ├── students.json      # 学生数据存储
│   ├── devices.json       # 设备绑定数据存储
│   ├── uploads/           # 音频文件存储目录
│   ├── templates/         # Web模板
...
- `POST /api/students` - 添加学生（可选`device_id`）
- `POST /api/devices` - 创建设备绑定
- `DELETE /api/students/<name>` - 删除学生及文件
